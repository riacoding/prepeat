...
export default function LinkAccount() {
  ...
  const { id, businessName, handle } = useMerchant()
  const router = useRouter()

  useEffect(() => {
    async function getUrl() {
      if (id) {
        const { url, state } = await getAuthUrl(id)
        setUrl(url)
        setAuthState(state)
  ...
  return (
    <div className='max-w-md mx-auto space-y-6 border p-6 rounded-lg shadow-sm bg-white'>
      ...
      <div className='text-center'>
        ...
        <Button
          size='lg'
          className='w-full bg-prepeat-orange hover:bg-orange-600'
          disabled={isConfiguring || !url}
          onClick={() => {
